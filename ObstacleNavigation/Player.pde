class Player {
  ...
  void calculateFitness() {
    if(reachedFinish) {
       fitness = 1000000.0/(float)(steps*steps); 
    } else {
  ...
  float lookInDirection(PVector direction) {
     ...
     while(distance < 60) {
         if(wall.checkCollision(position.x,position.y)) {
            return 1/distance; 
         } else if(position.x >= width || position.x <= 0 || position.y >= height || position.y <= 0) {
            return 1/distance; 
         }
  ...
  void think() {
     ...
     if(decision[0] > 0.8) {
       moveUp();
     } else if(decision[1] > 0.8) {
        moveDown(); 
     } else if(decision[2] > 0.8) {
         moveLeft();
     } else if(decision[3] > 0.8) {
        moveRight(); 
     } else if(decision[4] > 0.8) {
       moveUpLeft();
     } else if(decision[5] > 0.8) {
       moveDownLeft();
     } else if(decision[6] > 0.8) {
        moveUpRight();
     } else if(decision[7] > 0.8) {
        moveDownRight(); 
     }
  }  
